...
// ─── Helper: Get Layer Label ────────────────────────────────
const getLayerLabel = (layerKey) => {
  const layer = Number(layerKey);

  if (layer === 1) return `Layer ${layer} (You)`;
  if (layer === 2) return `Layer ${layer} (Direct)`;
...
export const buildTeamPDF = (pdf, { layersData, name, username }) => {
  ...
  const summaryData = Object.keys(layersData)
    ...
    .map((layerKey) => {
      ...
      const inactiveCount = layer?.inactive?.length || 0;
      return {
        layer: getLayerLabel(layerKey), // ← "Layer 1 (You)" for layer 1
        active: activeCount.toString(),
        inactive: inactiveCount.toString(),
        total: (activeCount + inactiveCount).toString(),
      };
  ...
  sortedLayers.forEach((layerKey) => {
    ...
    pdf.addSectionTitle(getLayerLabel(layerKey)); // ← "Layer 1 (You)" for layer 1
