...
const useSocket = ({
  ...
  const decryptMsg = async (raw) => {
    if (raw && typeof raw === "object" && raw.cipherText) {
      try {
        return await decryptMessage(raw, SECRET_KEY);
      } catch {
  ...
  const connectSocket = () => {
    ...
    // ── FIX: never send date=undefined to the server ──────────────────────
    // If registeredDate is missing, omit the param entirely so the server
    // doesn't reject the handshake with a bad query string.
    const socketQuery = { userId: currentUser.id };
    if (
      currentUser.userregisteredDate !== undefined &&
      currentUser.userregisteredDate !== null
    ) {
    ...
    socketRef.current = io(socketUrl, {
      transports: ["websocket"],
      query: socketQuery,
      // ── FIX: reconnection with backoff prevents the rapid retry storm ──
      reconnection: true,
      ...
      const msgObj = normalizeMsg(data, {
        msgBody: {
          ...
          media: data.msgBody?.media
            ? { ...data.msgBody.media, is_uploading: false }
            : undefined,
        },
      });
    ...
    socket.on("send_message", async (data) => {
      ...
      const decrypted = await decryptMsg(data.msgBody?.message);
      const msgObj = normalizeMsg(data, {
        msgBody: { ...data.msgBody, message: decrypted },
      });
      const isMe = data.fromUserId === currentUser.id;
    ...
    socket.on(
      ...
      ({ error, tempId, correlationId, msgId }) => {
        const isRateLimit = /rate limit|too many|slow down|wait/i.test(
          error || "",
        );
      ...
      const formatted = await Promise.all(
        chatMessages.map(async (msg) => {
          ...
          return normalizeMsg(msg, {
            msgBody: { ...msg.msgBody, message: decrypted },
          });
        }),
      );
    ...
    socket.on("report_received", (data) => {
      if (data.success) console.info("Report submitted");
    });
  };
