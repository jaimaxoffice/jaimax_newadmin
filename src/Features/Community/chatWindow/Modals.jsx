...
import {
    Trash2, AlertTriangle, X, Send, File, Image as ImageIcon, Eye, CheckCheck
} from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowErrorDetail(null)}>
            <div className="bg-[#202c33] rounded-lg max-w-sm w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="p-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            onClick={() => { setShowErrorDetail(null); retryMessage?.(showErrorDetail); }}
...
// ── File Type Modal ───────────────────────────────────────────────────────────
export const FileTypeModal = ({ showFileTypeModal, setShowFileTypeModal, fileInputRef }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
            <div className="w-full max-w-sm rounded-2xl bg-[#b9fd5c] border border-[#1e7f85] p-6">
                <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={cancelFileUpload} className="p-2 rounded-full hover:bg-white/10 transition">
                        <X className="w-5 h-5 text-black" />
                    </button>
                </div>
...
export const ImagePreviewModal = ({
    ...
    uploadingFile, cancelImageUpload, sendImageMessage, removeImage, formatFileSize
}) => {
                ...
                <div className="max-w-4xl mx-auto space-y-3">
                    ...
                    >
                        {uploadingFile
                            ...
                            : (<><Send className="w-5 h-5" />Send {selectedImages.length} {selectedImages.length === 1 ? 'Photo' : 'Photos'}</>)
                        }
                    </button>
                </div>
            </div>
            ...
            <div className="bg-[#202c33] rounded-lg w-full max-w-md shadow-2xl">
                ...
                <div className="p-6">
                    ...
                    >
                        {uploadingFile
                            ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />Sending...</>)
                            : (<><Send className="w-5 h-5" />Send Document</>)
                        }
                    </button>
                </div>
            </div>
...
export const ReportModal = ({
    showReportModal, reportingMessage, reportReason, reportDescription,
    setReportReason, setReportDescription, setShowReportModal, setReportingMessage,
    groupKey, submitReport
}) => {
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-2">
            <div className="bg-[#202c33] rounded-lg w-full max-w-sm">
                <div className="p-3 border-b border-[#2a3942] flex items-center justify-between">
                    ...
                    <button
                        onClick={() => { setShowReportModal(false); setReportingMessage(null); setReportReason(''); setReportDescription(''); }}
                    ...
                    <div>
                        ...
                        <div className="space-y-1">
                            {[
                                ...
                                <label
                                    key={option.value}
                                    className={`flex items-center gap-2 p-2 rounded-md cursor-pointer text-xs ${reportReason === option.value ? 'bg-[#00a884]/20 border border-[#00a884]' : 'bg-[#0b141a] hover:bg-[#1a2730]'}`}
                                >
                                    <input
                                        type="radio"
                                        name="reportReason"
...
// ── Read Receipts Modal ───────────────────────────────────────────────────────
export const ReadReceiptsModal = ({ showReadReceipts, setShowReadReceipts, selectedMessageForReceipts, members, formatTime }) => {
        ...
        <div className="absolute inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
            <div className="bg-[#202c33] rounded-lg max-w-md w-full max-h-[80vh] flex flex-col">
                <div className="p-4 border-b border-[#2a3942] flex items-center justify-between">
                    ...
                    <button onClick={() => setShowReadReceipts(false)} className="text-gray-400 hover:text-white">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                <div className="flex-1 overflow-y-auto p-4">
