...
//     if (messageText === null || messageText === undefined || messageText === 0 || messageText === "0") return "";
//     if (typeof messageText === "number") return "";  // ← ADD THIS LINE
...
//       if (/^\d+$/.test(t)) return false;  // ← ADD: filter pure numbers like "0", "00", "123"
...
//               {isFile &&
//                 decryptedText &&
//                 decryptedText.trim().length > 1 &&          // ← ADD
//                 !/^\d+$/.test(decryptedText.trim()) &&       // ← ADD: skip pure numbers
...
const MessageBubble = ({
  ...
  // Simple text resolver — if it's a file message, return empty string always
  const decryptedText = useMemo(() => {
    if (isFile) return "";
    const messageText = msg.msgBody?.message;
    ...
    if (typeof messageText === "object") {
      if (isEncryptedObject(messageText) && groupKey) {
        try {
          const decrypted = decryptMessage(messageText, groupKey);
          if (typeof decrypted === "string" && decrypted.trim().length >= 2)
            return decrypted.trim();
        } catch {}
        return "";
      }
    ...
    return "";
  }, [msg._id, msg.msgBody?.message, groupKey, isFile]);
          ...
          {!msg.deletedForEveryone && !isReported && (
            <div className="flex items-center justify-end gap-1 mt-1">
              {isPinned && (
                <Pin
                  className="w-2.5 h-2.5 text-amber-400/50 rotate-45"
                  title={`Pinned${getPinnerName() ? ` by ${getPinnerName()}` : ""}${msg.pinnedAt ? ` · ${formatPinTime(msg.pinnedAt)}` : ""}`}
                />
              )}
              {(isEdited || msg.isEdited) && (
                <span className="text-[10px] text-gray-400 italic">
