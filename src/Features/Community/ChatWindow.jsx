...
// ─── Safe reply text helper ──────────────────────────────────────────────────
// Handles: encrypted object, plain string, undefined, accidental file imports
const safeReplyText = (raw, groupKey) => {
    if (!raw) return 'Media message';
    // If it's an encrypted object try to decrypt
    if (typeof raw === 'object' && raw !== null) {
        if (raw.cipherText) {
            try { return decryptMessage(raw, groupKey); } catch { return '[Encrypted]'; }
        }
        // Any other object → never show [object Object] or source code
        return 'Media message';
    }
    if (typeof raw === 'string') {
        // Guard: if the string looks like source code (>200 chars with "export"/"import"/"{}")
        // it's a corrupted value — show placeholder instead
        if (raw.length > 200 && (raw.includes('export ') || raw.includes('import ') || raw.includes('=>'))) {
            return '[Message]';
        }
        return raw;
    }
...
const ChatWindow = ({
    ...
    removeImage,
    formatFileSize,
    // Emoji refs from GroupChatApp fixes
    emojiButtonRef,
    emojiClickInsideRef,
}) => {
    ...
    const getFileUrl = (msg) => msg.msgBody?.media?.file_url || msg.fileUrl || msg.msgBody?.media?.fileUrl || null;
    const getFileType = (msg) => msg.msgBody?.media?.file_type || msg.msgBody?.media?.fileType || null;
    const getFileName = (msg) => msg.msgBody?.media?.fileName || msg.msgBody?.message || 'File';
    const getFileSize = (msg) => msg.msgBody?.media?.file_size || msg.msgBody?.media?.fileSize || null;
    ...
    useEffect(() => {
        ...
        if (isInputDisabled) {
            ...
            timer = setInterval(() => {
                setCountdown((prev) => {
                    if (prev <= 1) { clearInterval(timer); setIsInputDisabled(false); return 0; }
                    return prev - 1;
                });
            }, 1000);
        }
    ...
    const canSendMessages = (role) => role === 0 || role === 2;
    const isRoleDisabled = !canSendMessages(userRole);
    ...
    useEffect(() => { setIsComponentLoading(true); }, [selectedGroup?.id]);
    ...
    const downloadFileToDesktop = async (fileUrl, fileName) => {
        try {
            ...
            const link = document.createElement('a');
            link.href = fileUrl; link.download = fileName || 'download';
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            scrollTimeout = setTimeout(() => {
                const { scrollTop, scrollHeight, clientHeight } = container;
                const scrollPercentage = ((scrollTop + clientHeight) / scrollHeight) * 100;
                if (scrollPercentage > 90 && userPage < totalPages && !isLoadingUsers) loadNextPage?.();
                if (scrollPercentage < 10 && userPage > 1 && !isLoadingUsers) loadPrevPage?.();
            }, 300);
        };
        container.addEventListener('scroll', handleScroll);
    ...
    useEffect(() => {
        const handleClickOutside = () => { if (showHeaderMenu) setShowHeaderMenu(false); };
    ...
    useEffect(() => {
        if (selectedGroup && currentUser) {
            const welcomeKey = `jaimx_welcome_${currentUser.id}_${selectedGroup.id}`;
            setShowWelcomeMessage(!Cookies.get(welcomeKey));
        }
    }, [selectedGroup?.id, currentUser?.id]);
    ...
    const submitReport = () => {
        ...
        reportMessage?.(reportData);
        setShowReportModal(false); setReportingMessage(null); setReportReason(''); setReportDescription('');
    };
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            if (isAtBottomRef.current) setNewMessagesCount(0);
            scrollTimeout = setTimeout(() => {
                if (distanceFromTop < 200 && hasMoreOldMessagesRef.current && !isLoadingOlderRef.current) loadOlderMessages();
                if (distanceFromBottom < 200 && hasMoreNewMessages && !isLoadingNewer && !isAtBottomRef.current) loadNewerMessages();
            }, 200);
        };
        messagesArea.addEventListener('scroll', handleScroll);
        return () => { clearTimeout(scrollTimeout); messagesArea.removeEventListener('scroll', handleScroll); };
    ...
    const handleEmojiClick = (emojiObject) => {
        setMessage(prev => prev + emojiObject.emoji);
        // DO NOT call setShowEmojiPicker(false) — picker stays open for multiple picks
    ...
    useEffect(() => {
        if (!isLoadingOlder && scrollPositionRef.current && isRestoringScrollRef.current) {
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    const targetElement = document.getElementById(`msg-${scrollPositionRef.current}`);
                    if (targetElement) targetElement.scrollIntoView({ block: 'start', behavior: 'instant' });
                    scrollPositionRef.current = null;
                    isRestoringScrollRef.current = false;
                });
            });
    ...
    useEffect(() => {
        ...
        observerRef.current = new IntersectionObserver((entries) => {
            ...
            entries.forEach((entry) => {
                ...
                setObservedMessages(prev => { const s = new Set(prev); s.add(msgId); return s; });
                onMarkAsRead?.({ msgId, groupId: selectedGroup.id, chatId: selectedGroup.chatId, userId: currentUser.id, timestamp: now });
                if (socketRef?.current?.connected) {
                    socketRef.current.emit("message:read", { msgId, chatId: selectedGroup.chatId, userId: currentUser.id, readAt: now });
                }
            });
        }, { root: messagesAreaRef.current, threshold: 0.7, rootMargin: "-20px 0px -20px 0px" });
    ...
    useEffect(() => { setObservedMessages(new Set()); }, [selectedGroup?.id]);

    useEffect(() => {
        const messagesArea = messagesAreaRef.current;
        if (!messagesArea || !messages?.length) return;
        if (isRestoringScrollRef.current || isLoadingOlder) { prevMessagesLengthRef.current = messages.length; return; }
        if (isInitialLoad) {
            if (!isInitialMessagesLoad && !isLoadingMessages) {
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => { messagesEndRef.current?.scrollIntoView({ behavior: 'instant' }); isAtBottomRef.current = true; });
                });
                setIsInitialLoad(false);
            }
            prevMessagesLengthRef.current = messages.length;
        ...
        const lastMessage = messages[currentLength - 1];
        const previousLastMessage = prevLength > 0 ? messages[prevLength - 1] : null;
        if (previousLastMessage && lastMessage.msgId === previousLastMessage.msgId) { prevMessagesLengthRef.current = currentLength; return; }
        const isMyMessage = lastMessage?.fromUserId?.toString() === currentUser?.id?.toString();
        if (isMyMessage) {
    ...
    const toggleMenu = (msgId, e, isCurrentUser) => {
        e?.stopPropagation();
        if (effectiveOpenMenuId === msgId) { setEffectiveOpenMenuId(null); return; }
        const msgElem = document.getElementById(`msg-${msgId}`);
        const container = containerRef.current;
        const header = headerRef.current;
        if (!msgElem || !container) {
            setMenuPosition({ top: 80, left: Math.max(16, (container?.clientWidth || 300) - 220) });
            setEffectiveOpenMenuId(msgId); return;
        }
        const msgRect = msgElem.getBoundingClientRect();
        const containerRect = container.getBoundingClientRect();
        const headerRect = header ? header.getBoundingClientRect() : { bottom: containerRect.top };
        const spaceAbove = msgRect.top - containerRect.top - (headerRect.height || 0);
        const spaceBelow = containerRect.bottom - msgRect.bottom;
        const MENU_W = 180, MENU_H = 160, GAP = 8;
        let left = isCurrentUser
            ? Math.max(containerRect.left + 8, Math.min(msgRect.right - MENU_W, containerRect.right - 8 - MENU_W))
            : Math.max(containerRect.left + 8, Math.min(msgRect.left, containerRect.right - 8 - MENU_W));
        let top = spaceAbove > MENU_H + GAP ? msgRect.top - MENU_H - GAP
            : spaceBelow > MENU_H + GAP ? msgRect.bottom + GAP
                : Math.min(Math.max(containerRect.top + 8, msgRect.bottom + GAP), containerRect.bottom - MENU_H - 8);
        setMenuPosition({ top: Math.round(top - containerRect.top), left: Math.round(left - containerRect.left) });
        setEffectiveOpenMenuId(msgId);
    };
    ...
    useEffect(() => {
        const onKey = (e) => { if (e.key === 'Escape') { setEffectiveOpenMenuId(null); setShowReadReceipts(false); } };
        window.addEventListener('keydown', onKey);
        return () => window.removeEventListener('keydown', onKey);
    }, []);
    ...
    const handleCopyMessage = (messageText, id) => {
        ...
        if (typeof messageText === 'object' && messageText !== null) {
            if (messageText.cipherText && groupKey) {
                try { textToCopy = decryptMessage(messageText, groupKey); }
                catch { textToCopy = "[Encrypted message]"; }
            } else { textToCopy = "[Unable to copy]"; }
        } else if (typeof messageText === 'string') {
            textToCopy = messageText;
        } else { textToCopy = String(messageText || ''); }
        navigator.clipboard.writeText(textToCopy)
            .then(() => { setCopiedMessageId(id); setTimeout(() => setCopiedMessageId(null), 2000); })
            .catch((err) => console.error("Failed to copy:", err));
    };
        ...
        <>
            {isComponentLoading ? (
                ...
                <div ref={containerRef} className="flex-1 flex flex-col relative h-full sidebar-scroll">
                    ...
                    {/* ── HEADER ── */}
                    {selectedGroup && (
                        <div ref={headerRef} className="bg-[#b9fd5c] p-2 sm:p-4 flex items-center justify-between border-b border-[#2a3942] flex-shrink-0">
                            <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                                ...
                                >
                                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#00a884] to-[#008069] flex items-center justify-center text-xl flex-shrink-0">
                                        <img src="https://res.cloudinary.com/ddefr5owc/image/upload/v1766049897/logo_xwrr9w.png" alt="" className="w-full h-full rounded-full object-cover" />
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <h2 className="font-semibold text-black truncate">{selectedGroup?.name}</h2>
                                        {typingUsers?.length > 0 ? (
                                            <div className="flex items-center gap-1">
                                                ...
                                                <div className="flex gap-0.5">
                                                    <span className="w-1 h-1 bg-[#00a884] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                                                    <span className="w-1 h-1 bg-[#00a884] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                                                    <span className="w-1 h-1 bg-[#00a884] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                                                </div>
                                            </div>
                                        ) : (
                                            <p className="text-xs text-black truncate">{totalUsers ?? 0} members</p>
                            ...
                            {isInitialMessagesLoad && isLoadingMessages ? (
                                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#111b21]/80 z-50">
                                    <div className="relative w-20 h-20">
                                        ...
                                        <div className="absolute inset-0 rounded-full border-4 border-transparent border-b-[#00a884]/40 animate-spin"
                                            style={{ animationDirection: "reverse", animationDuration: "1s" }} />
                                        <img src={loaderImage} alt="loader" className="absolute inset-0 m-auto w-10 h-10 object-contain" />
                                    </div>
                                    <p className="text-lg mt-6 mb-1 font-medium text-gray-300">Loading messages…</p>
                                                ...
                                                <div key={dateKey}>
                                                    ...
                                                    {dateMessages.map((msg) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                {/*
                                                                    FIX: max-w-[75%] on THIS wrapper div constrains bubble width.
                                                                    min-w-0 is critical — without it flex children don't shrink.
                                                                    overflow-hidden prevents any child from blowing out the width.
                                                                */}
                                                                <div className={`
                                                                    relative
                                                                ...
                                                                `}>
                                                                    ...
                                                                    {/* ── Reply preview ─────────────────────────────────────
                                                                        FIX: use safeReplyText() helper which handles all edge
                                                                        cases (object, encrypted, source code strings, undefined).
                                                                        Strictly truncated to 2 lines max.
                                                                    */}
                                                                    {msg.replyTo && (
                                                                        <div
                                                                            className="mb-2 p-2 bg-black/20 rounded border-l-4 border-[#00a884] cursor-pointer hover:bg-black/30 overflow-hidden"
                                                                            onClick={() => scrollToMessage(msg.replyTo.msgId)}
                                                                        >
                                                                            <div className="text-xs text-[#00a884] font-semibold mb-0.5 truncate">
                                                                                {msg.replyTo.senderName || msg.fromUserId || 'User'}
                                                                            </div>
                                                                            <div className="text-xs text-gray-400 line-clamp-2 break-words">
                                                                                {safeReplyText(msg.replyTo.message, groupKey)}
                                                                            </div>
                                                                        </div>
                                                                    )}
                                                                        ...
                                                                        <div className="relative group max-w-xs">
                                                                            <div className="relative rounded-lg overflow-hidden">
                                                                                <img
                                                                                    ...
                                                                                    onClick={() => {
                                                                                        if (!msg.msgBody?.media?.is_uploading && getFileUrl(msg) && !getFileUrl(msg).startsWith('blob:')) {
                                                                                            window.open(getFileUrl(msg), '_blank');
                                                                                        }
                                                                                    }}
                                                                                />
                                                                    ...
                                                                    {!msg.deletedForEveryone && getFileUrl(msg) && !getFileType(msg)?.startsWith('image/') && (
                                                                        <div className="max-w-xs">
                                                                            <div
                                                                                className={`group relative flex items-center gap-3 p-3 rounded-xl bg-gradient-to-br from-[#1a2332] to-[#0f1419] border transition-all ${msg.msgBody?.media?.is_uploading ? 'opacity-60' : 'hover:scale-[1.02] cursor-pointer'}`}
                                                                                onClick={() => { if (!msg.msgBody?.media?.is_uploading && getFileUrl(msg)) window.open(getFileUrl(msg), '_blank'); }}
                                                                            >
                                                                    ...
                                                                    {/* ── Message text ──────────────────────────────────────
                                                                        FIX: break-words + whitespace-pre-wrap prevent long
                                                                        unbroken strings (URLs, code) from stretching the bubble.
                                                                        overflow-wrap: anywhere as inline style catches edge cases.
                                                                    */}
                                                                    {msg.deletedForEveryone ? (
                                                                            ...
                                                                            {!getFileUrl(msg) && (
                                                                                ...
                                                                                >
                                                                                    {msg.isreported?.count >= 3 && msg.isreported?.isHidden ? (
                                                                                        <span className="italic text-gray-400">Reported content hidden</span>
                                                                                    ) : (
                                                                                        renderMessageWithLinks(decryptedText)
                                                                                    )}
                                                                                </div>
                                                                    ...
                                                                    {/* Timestamp + menu */}
                                                                    <div className="flex items-center justify-end gap-1 mt-1 text-xs text-gray-400">
                                                                        <span className="text-white text-[10px]">{formatTime(msg.timestamp)}</span>
                                                                        {!msg.deletedForEveryone && (
                                                                            <button
                                                                                className="ml-1 p-1 text-white hover:bg-white/20 rounded-full"
                                                                                onClick={(e) => toggleMenu(id, e, isCurrentUser)}
                    ...
                    {showImagePreview && selectedImages.length > 0 && (
                        <div className="fixed inset-0 bg-black z-50 flex flex-col">
                            <div className="bg-[#202c33] p-4 flex items-center justify-between border-b border-[#2a3942]">
                                <button onClick={cancelImageUpload} className="text-white hover:bg-white/10 p-2 rounded-full transition">
                                    <X className="w-6 h-6" />
                                </button>
                                <h3 className="text-lg font-semibold text-white">
                                    {selectedImages.length} {selectedImages.length === 1 ? 'Photo' : 'Photos'}
                            ...
                            <div className="bg-[#202c33] p-4 border-t border-[#2a3942]">
                                <div className="max-w-4xl mx-auto space-y-3">
                                    <input type="text" value={imageCaption} onChange={(e) => setImageCaption(e.target.value)}
                                        ...
                                        className="w-full px-4 py-3 bg-[#2a3942] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a884] placeholder-gray-400"
                                        maxLength={1000} />
                                    <button onClick={sendImageMessage} disabled={uploadingFile}
                                        className="w-full bg-[#00a884] hover:bg-[#008069] disabled:bg-gray-600 text-white py-3 rounded-lg font-semibold text-lg transition-colors flex items-center justify-center gap-2">
                                        {uploadingFile ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />Sending...</>) : (<><Send className="w-5 h-5" />Send {selectedImages.length} {selectedImages.length === 1 ? 'Photo' : 'Photos'}</>)}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {showDocumentPreview && selectedDocument && (
                        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
                            <div className="bg-[#202c33] rounded-lg w-full max-w-md shadow-2xl">
                                <div className="p-4 border-b border-[#2a3942] flex items-center justify-between">
                                    <h3 className="text-lg font-semibold text-white">Send Document</h3>
                                    <button onClick={cancelDocumentUpload} className="text-gray-400 hover:text-white"><X className="w-6 h-6" /></button>
                                </div>
                                <div className="p-6">
                                    <div className="flex items-center gap-4 p-4 bg-[#0b141a] rounded-lg mb-4">
                                        <div className="w-16 h-16 rounded-lg bg-blue-500/20 flex items-center justify-center text-4xl flex-shrink-0">
                                    ...
                                    <button onClick={sendDocumentMessage} disabled={uploadingFile}
                                        className="w-full bg-[#00a884] hover:bg-[#008069] disabled:bg-gray-600 text-white py-3 rounded-lg font-semibold text-lg transition-colors flex items-center justify-center gap-2">
                                        {uploadingFile ? (<><div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white" />Sending...</>) : (<><Send className="w-5 h-5" />Send Document</>)}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* ── ERROR DETAIL MODAL ── */}
                    {showErrorDetail && (
                        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={() => setShowErrorDetail(null)}>
                            <div className="bg-[#202c33] rounded-lg max-w-sm w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
                                <div className="p-4 border-b border-[#2a3942]">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-semibold text-white">Message Not Sent</h3>
                                            <p className="text-xs text-gray-400">{formatTime(showErrorDetail.timestamp)}</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="p-4">
                                    <div className="mb-4">
                                        <p className="text-xs text-gray-400 mb-2">Error reason:</p>
                                        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3">
                                            <p className="text-sm text-red-400">{showErrorDetail.error || "Unknown error occurred"}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => { setShowErrorDetail(null); retryMessage?.(showErrorDetail); }}
                                            className="flex-1 bg-[#00a884] hover:bg-[#008069] text-white py-2.5 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2">
                                            Retry
                                        </button>
                                        <button onClick={() => setShowErrorDetail(null)} className="flex-1 bg-[#0b141a] hover:bg-[#1a2730] text-white py-2.5 rounded-lg font-medium text-sm transition-colors">Close</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {showFileTypeModal && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
                            <div className="w-full max-w-sm rounded-2xl bg-[#b9fd5c] border border-[#1e7f85] p-6">
                                ...
                                <div className="space-y-3">
                                    <button onClick={() => { fileInputRef?.current?.setAttribute("accept", "image/*"); fileInputRef?.current?.setAttribute("multiple", "true"); fileInputRef?.current?.click(); setShowFileTypeModal(false); }}
                                        className="group w-full flex items-center gap-4 rounded-xl bg-[#000000] p-4 transition active:scale-[0.97]">
                                        <div className="w-12 h-12 rounded-full bg-[#b9fd5c] flex items-center justify-center">
                                            <ImageIcon className="w-6 h-6 text-gray-500" />
                                        </div>
                                    ...
                                    <button onClick={() => { fileInputRef?.current?.setAttribute("accept", "application/pdf,.doc,.docx,.txt,.xlsx,.xls,.ppt,.pptx,.zip,.rar"); fileInputRef?.current?.removeAttribute("multiple"); fileInputRef?.current?.click(); setShowFileTypeModal(false); }}
                                        className="group w-full flex items-center gap-4 rounded-xl bg-[#000000] p-4 transition active:scale-[0.97]">
                                        <div className="w-12 h-12 rounded-full bg-[#b9fd5c] flex items-center justify-center">
                                            <File className="w-6 h-6 text-gray-500" />
                                        </div>
                    ...
                    {selectedGroup && effectiveOpenMenuId && (
                        <div
                            ...
                            onClick={(e) => e.stopPropagation()}
                        >
                            <button className="flex items-center gap-3 px-4 py-3 hover:bg-[#3b4a54] w-full text-left text-sm text-white transition-colors"
                                onClick={() => { const msg = messages?.find(m => (m.msgId || m.id) === effectiveOpenMenuId); if (msg) { handleReply(msg); setEffectiveOpenMenuId(null); } }}>
                                <CornerUpLeft className="w-4 h-4 text-[#00a884]" /><span>Reply</span>
                            </button>
                            {messages?.find(m => (m.msgId || m.id) === effectiveOpenMenuId)?.msgBody?.message && (
                                <button className="flex items-center gap-3 px-4 py-3 hover:bg-[#3b4a54] w-full text-left text-sm text-white transition-colors border-t border-[#3b4a54]"
                                    onClick={() => handleCopyMessage(messages.find(m => (m.msgId || m.id) === effectiveOpenMenuId).msgBody.message, effectiveOpenMenuId)}>
                                    <Clipboard className="w-4 h-4 text-blue-400" />
                                    <span>{copiedMessageId === effectiveOpenMenuId ? "Copied ✓" : "Copy"}</span>
                                </button>
                            )}
                            {messages?.find(m => (m.msgId || m.id) === effectiveOpenMenuId)?.status === "failed" && (
                                <button className="flex items-center gap-3 px-4 py-3 hover:bg-[#3b4a54] w-full text-left text-sm text-orange-400 transition-colors border-t border-[#3b4a54]"
                                    onClick={() => { const msg = messages.find(m => (m.msgId || m.id) === effectiveOpenMenuId); if (msg) { retryMessage(msg); setEffectiveOpenMenuId(null); } }}>
                                    <CornerUpRight className="w-4 h-4" /><span>Retry</span>
                                </button>
                            )}
                            <button className="flex items-center gap-3 px-4 py-3 hover:bg-[#3b4a54] w-full text-left text-sm text-red-400 transition-colors border-t border-[#3b4a54]"
                                onClick={() => { const msg = messages.find(m => (m.msgId || m.id) === effectiveOpenMenuId); if (msg) { deleteForMe(msg._id?.toString() || msg.msgId || msg.id); setEffectiveOpenMenuId(null); } }}>
                            ...
                            {(() => {
                                ...
                                if (isMyMessage || isAdmin) return (
                                    <button className="flex items-center gap-3 px-4 py-3 hover:bg-[#3b4a54] w-full text-left text-sm text-red-400 transition-colors border-t border-[#3b4a54]"
                                        onClick={() => { if (msg) { deleteForEveryone(msg._id?.toString() || msg.msgId || msg.id); setEffectiveOpenMenuId(null); } }}>
                    ...
                    {showReportModal && reportingMessage && (
                        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-2">
                            <div className="bg-[#202c33] rounded-lg w-full max-w-sm">
                                <div className="p-3 border-b border-[#2a3942] flex items-center justify-between">
                                    <h3 className="text-sm font-semibold text-white">Report Message</h3>
                                    <button onClick={() => { setShowReportModal(false); setReportingMessage(null); setReportReason(''); setReportDescription(''); }} className="text-gray-400 hover:text-white">
                                        <X className="w-5 h-5" />
                                    </button>
                                </div>
                                <div className="p-3 space-y-3">
                                    <div className="p-2 bg-[#0b141a] rounded-md border border-[#2a3942]">
                                        ...
                                        <p className="text-xs text-gray-200 break-words line-clamp-3">
                                            {safeReplyText(reportingMessage.msgBody?.message, groupKey)}
                                        </p>
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-300 mb-1">Why are you reporting this? *</label>
                                        <div className="space-y-1">
                                            {[{ value: 'spam', label: 'Spam' }, { value: 'harassment', label: 'Harassment' }, { value: 'violence', label: 'Violence' }, { value: 'inappropriate', label: 'Inappropriate' }, { value: 'other', label: 'Other' }].map((option) => (
                                                <label key={option.value} className={`flex items-center gap-2 p-2 rounded-md cursor-pointer text-xs ${reportReason === option.value ? 'bg-[#00a884]/20 border border-[#00a884]' : 'bg-[#0b141a] hover:bg-[#1a2730]'}`}>
                                                    <input type="radio" name="reportReason" value={option.value} checked={reportReason === option.value} onChange={(e) => setReportReason(e.target.value)} className="w-3 h-3 text-[#00a884]" />
                                                    <span className="text-gray-200">{option.label}</span>
                                                </label>
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex gap-2 pt-1">
                                        <button onClick={() => { setShowReportModal(false); setReportingMessage(null); setReportReason(''); setReportDescription(''); }}
                                            className="flex-1 py-2 bg-[#0b141a] hover:bg-[#1a2730] text-xs text-white rounded-md">Cancel</button>
                    ...
                    {showReadReceipts && selectedMessageForReceipts && (
                        <div className="absolute inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
                            <div className="bg-[#202c33] rounded-lg max-w-md w-full max-h-[80vh] flex flex-col">
                                <div className="p-4 border-b border-[#2a3942] flex items-center justify-between">
                                    <h3 className="text-lg font-semibold">Read by</h3>
                                    <button onClick={() => setShowReadReceipts(false)} className="text-gray-400 hover:text-white"><X className="w-6 h-6" /></button>
                                </div>
                                <div className="flex-1 overflow-y-auto p-4">
                                    {selectedMessageForReceipts.metaData?.readBy?.length > 0 ? (
                                        <div className="space-y-3">
                    ...
                    {showFilePreview && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4">
                            <div className="w-full max-w-sm rounded-2xl bg-[#b9fd5c] border border-[#1e7f85] p-6">
                                <div className="flex items-center justify-between mb-5">
                                    <h3 className="text-lg font-semibold text-black">Send File</h3>
                                    <button onClick={cancelFileUpload} className="p-2 rounded-full hover:bg-white/10 transition"><X className="w-5 h-5 text-black" /></button>
                                </div>
                                {selectedFile?.type?.startsWith("image/") && filePreview ? (
                                    <div className="w-full rounded-xl overflow-hidden mb-4 bg-[#000000] p-3">
                                        <img src={filePreview} alt="Preview" className="w-full h-48 object-contain rounded-lg" />
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-4 p-4 bg-[#000000] rounded-xl w-full mb-4">
                                        <div className="w-12 h-12 rounded-full bg-[#b9fd5c] flex items-center justify-center flex-shrink-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="font-medium text-base text-white truncate">{selectedFile?.name}</div>
                                            <div className="text-sm text-[#d1f5f7]">{(selectedFile?.size / 1024 / 1024).toFixed(2)} MB</div>
                                        </div>
                                    </div>
                                )}
                                <button onClick={sendFileMessage} disabled={uploadingFile}
                                    className="w-full bg-[#000000] disabled:opacity-50 text-white py-3 rounded-xl font-medium text-base transition active:scale-[0.97]">
                                    {uploadingFile ? 'Uploading...' : 'Upload File'}
                                </button>
                            </div>
                        </div>
                    ...
                    {/* ── REPLY BAR ── */}
                    {replyToMessage && selectedGroup && !showMembers && !showFilesPanel && (
                        <div className="bg-[#2a3942] px-3 py-2 border-t border-[#3b4a54] flex items-center gap-3 flex-shrink-0">
                            <div className="flex-1 min-w-0 border-l-4 border-[#00a884] pl-3">
                                <p className="text-xs text-[#00a884] font-semibold truncate">
                                    {replyToMessage.publisherName || replyToMessage.senderName || replyToMessage.fromUserId}
                                </p>
                                <p className="text-xs text-gray-400 truncate">
                                    {safeReplyText(replyToMessage.msgBody?.message, groupKey)}
                                </p>
                            </div>
                            <button onClick={cancelReply} className="p-1 hover:bg-white/20 rounded-full transition-colors flex-shrink-0" aria-label="Cancel reply">
                                <X className="w-5 h-5 text-gray-400" />
                            </button>
                        </div>
                    )}
                    ...
                    {/* ── INPUT BAR ── */}
                    {selectedGroup && !showMembers && !showFilesPanel && (
                        <div className="bg-[#202c33] p-2 sm:p-3 border-t border-[#ffffff] z-20 flex-shrink-0">
                            {isInputDisabled && (
                                <div className="mb-2 rounded-xl px-4 py-2.5 flex items-center gap-3 border border-teal-500/30 bg-[#000000]">
                                    <div className="flex-shrink-0 w-7 h-7 rounded-full bg-[#000000] border border-teal-500/30 flex items-center justify-center">
                                        <AlertTriangle className="w-3.5 h-3.5 text-white" />
                            ...
                            <div className="flex items-center gap-1 sm:gap-2 relative">
                                {/* ── FIX: emoji toggle uses emojiButtonRef + pointerdown so it doesn't
                                     interfere with the outside-click handler in GroupChatApp ── */}
                                <button
                                    ref={emojiButtonRef}
                                    onPointerDown={(e) => {
                                        e.stopPropagation();
                                ...
                                {/* ── FIX: picker wrapper uses onPointerDown to set emojiClickInsideRef flag
                                     so the document-level close handler ignores clicks inside the picker ── */}
                                {showEmojiPicker && !isInputDisabled && (
                                    <div
                                        ref={emojiPickerRef}
                                ...
                                <input
                                    ...
                                    disabled={isInputDisabled}
                                    placeholder="Type a message"
                                    className={`flex-1 min-w-0 px-3 sm:px-4 py-2 sm:py-3 bg-[#2a3942] text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#085358] placeholder-gray-400 text-sm sm:text-base transition-opacity ${isInputDisabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                                />
                    ...
                    {selectedGroup && showMembers && (
                        <div className="flex-1 flex flex-col bg-[#0b141a] h-full">
                            <div className="bg-[#202c33] p-2 sm:p-4 flex items-center gap-2 sm:gap-3 border-b border-[#2a3942] flex-shrink-0">
                                <button onClick={() => { setShowMembers(false); setActiveGroupTab('overview'); setTimeout(() => messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 100); }}
                                    className="hover:bg-[#0a6a72] p-1.5 sm:p-2 rounded-lg transition-colors flex-shrink-0">
                                    <ArrowLeft className="w-4 h-4 sm:w-5 sm:h-5" />
                                </button>
                                <h2 className="text-base sm:text-lg font-semibold truncate">Group Info</h2>
                            ...
                            <div className="bg-[#202c33] border-b border-[#2a3942] overflow-x-auto flex-shrink-0">
                                <div className="flex min-w-max">
                                    {[{ id: 'overview', icon: Info }, { id: 'media', icon: ImageIcon }, { id: 'files', icon: File }].map(({ id, icon: Icon }) => (
                                        <button key={id} onClick={() => { setActiveGroupTab(id); if (id !== 'overview') refetchFiles?.(); }}
                                            className={`flex-1 px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium border-b-2 transition-colors ${activeGroupTab === id ? "border-[#00a884] text-[#00a884]" : "border-transparent text-gray-400 hover:text-white"}`}>
                                            <div className="flex items-center justify-center gap-1.5 sm:gap-2">
                            ...
                            <div ref={membersContainerRef} className="flex-1 overflow-y-auto">
                                {activeGroupTab === 'overview' && (
                                    <div className="p-3 sm:p-4 space-y-3 sm:space-y-4">
                                        <div className="bg-[#202c33] rounded-lg p-3 sm:p-4">
                                            <h4 className="text-xs sm:text-sm font-semibold text-gray-400 mb-2 sm:mb-3">Group Description</h4>
                                            <p className="text-xs sm:text-sm text-gray-300 leading-relaxed">{selectedGroup?.groupDescription || 'No description available'}</p>
                                        </div>
                                        <div className="bg-[#202c33] rounded-lg p-3 sm:p-4">
                                            <h4 className="text-xs sm:text-sm font-semibold text-gray-400 mb-2 sm:mb-3">Group Details</h4>
                                ...
                                {activeGroupTab === 'media' && (
                                    <div className="p-3 sm:p-4">
                                        {loadingFiles ? (
                                            ...
                                            const allFiles = [];
                                            chatFiles?.forEach(cf => cf.files?.forEach(f => allFiles.push({ _id: cf._id, fileName: f.fileName, fileUrl: f.file_url, fileType: f.file_type, fileSize: f.file_size, senderName: cf.publisherName, timestamp: cf.timestamp || cf.createdAt })));
                                            const imageFiles = allFiles.filter(f => f.fileType?.startsWith('image/'));
                                            return imageFiles.length > 0 ? (
                                                <>
                                                    <h3 className="text-xs sm:text-sm text-gray-400 mb-2 sm:mb-3 font-semibold">{imageFiles.length} Images</h3>
                                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2">
                                                        {imageFiles.map((file, index) => (
                                                            <div key={`${file._id}-${index}`} className="relative group cursor-pointer aspect-square rounded-lg overflow-hidden bg-[#1f2f2a]" onClick={() => window.open(file.fileUrl, '_blank')}>
                                ...
                                {activeGroupTab === 'files' && (
                                    <div className="p-3 sm:p-4">
                                        {loadingFiles ? (
                                            ...
                                            return documentFiles.length > 0 ? (
                                                <>
                                                    <h3 className="text-xs sm:text-sm text-gray-400 mb-2 sm:mb-3 font-semibold">{documentFiles.length} Documents</h3>
                                                    <div className="space-y-1.5 sm:space-y-2">
                                                        {documentFiles.map((file, index) => (
                                                            <div key={`${file._id}-${index}`} className="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 bg-[#202c33] rounded-lg hover:bg-[#2a3942] transition-colors cursor-pointer" onClick={() => window.open(file.fileUrl, '_blank')}>
                                                                <div className="w-10 h-10 rounded bg-blue-500/20 flex items-center justify-center flex-shrink-0">
                                                                    <File className="w-6 h-6 text-blue-400" />
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                ...
                                                                <button onClick={async (e) => { e.preventDefault(); e.stopPropagation(); await downloadFileToDesktop(file.fileUrl, file.fileName); }}
                                                                    className="p-1.5 sm:p-2 hover:bg-[#0a6a72] rounded-full transition-colors flex-shrink-0">
                                                                    <Download className="w-4 h-4 sm:w-5 sm:h-5" />
                                                                </button>
                                                            </div>
                                ...
                                {loadingFiles ? (
                                    ...
                                    return (
                                        <div className="space-y-6">
                                            {imageFiles.length > 0 && (
                                                <div>
                                                    <h3 className="text-sm text-gray-400 mb-3 font-semibold flex items-center gap-2"><ImageIcon className="w-4 h-4" />Images ({imageFiles.length})</h3>
                                                    <div className="grid grid-cols-3 gap-2">
                                                        {imageFiles.map((file, index) => (
                                                            <div key={`${file._id}-${index}`} className="relative group cursor-pointer aspect-square rounded-lg overflow-hidden bg-[#1f2f2a]" onClick={() => window.open(file.fileUrl, '_blank')}>
                                            ...
                                            {documentFiles.length > 0 && (
                                                <div>
                                                    <h3 className="text-sm text-gray-400 mb-3 font-semibold flex items-center gap-2"><File className="w-4 h-4" />Documents ({documentFiles.length})</h3>
                                                    <div className="space-y-2">
                                                        {documentFiles.map((file, index) => (
                                                            <div key={`${file._id}-${index}`} className="flex items-center gap-3 p-3 bg-[#202c33] rounded-lg hover:bg-[#2a3942] transition-colors cursor-pointer" onClick={() => window.open(file.fileUrl, '_blank')}>
                                                                <div className="w-10 h-10 rounded bg-blue-500/20 flex items-center justify-center flex-shrink-0">
                                                                    <File className="w-6 h-6 text-blue-400" />
                                                                </div>
                                                                <div className="flex-1 min-w-0">
                                                                ...
                                                                <button onClick={(e) => { e.stopPropagation(); window.open(file.fileUrl, '_blank'); }} className="p-2 hover:bg-[#0a6a72] rounded-full transition-colors flex-shrink-0">
                                                                    <Download className="w-5 h-5" />
                                                                </button>
                                                            </div>
                                                        ))}
