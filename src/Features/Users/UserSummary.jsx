...
import { buildUserSummaryPDF } from "./buildUserSummaryPDF";
...
function UserSummary() {
    ...
    const handlePreviewPDF = async () => {
        if (!data?.data) return;
        await previewPDF(
            (pdf) => buildUserSummaryPDF(pdf, data?.data),
            {
                title: `User Report ${data?.data?.reqUser?.username || ""}`,
    ...
    return (
        <div className="min-h-screen font-mono bg-[#1a1f24] text-white">
            {/* Header */}
            <div className="sticky top-0 z-20 px-6 py-6 flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-[#1a1f24] border-b border-[#282f35]">
                {/* Search */}
                <form
                            ...
                            <button
                                ...
                                onClick={async () => {
                                    await downloadPDF(
                                        ...
                                        `UserSummary_${data?.data?.reqUser?.username}_${Date.now()}.pdf`,
                                    );
                                }}
                                className="px-4 py-2 rounded-full text-xs font-bold bg-[#b9fd5c] text-black"
                            >
                                {isGenerating ? "Generating..." : "Download PDF"}
                            </button>
                            <button
                                onClick={handlePreviewPDF}
                                className="px-4 py-2 rounded-full text-xs font-bold bg-[#1a1f24] border border-[#b9fd5c] text-[#b9fd5c]"
                            >
                                Preview PDF
                            </button>
                        </>
                ...
                {(isLoading || isFetching) && (
                    <div className="flex flex-col items-center justify-center py-32 gap-4">
                        ...
                        <span className="text-sm tracking-widest uppercase text-[#b9fd5c]">
                            {isLoading ? "Searching..." : "Refreshing..."}
                        </span>
                    </div>
                )}
                                        ...
                                        >
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span
                                                        className={`text-xs px-2 py-0.5 rounded-full capitalize ${inv.status === "completed"
