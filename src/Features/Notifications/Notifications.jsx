...
const BulkNotificationManagement = () => {
  ...
  // ── Lazy hooks — fire only when their trigger is called ──
  const [fetchAllUsers, allUsersResult] = useLazyGetAllUsersQuery();
  const [fetchInactiveUsers, inactiveResult] = useLazyGetInactiveUsersQuery();
  const [fetchZeroRefsUsers, zeroRefsResult] = useLazyGetUsersWithZeroDirectRefsQuery();
  const [fetchOneToTwoRefsUsers, oneToTwoResult] = useLazyGetUsersWithOneToTwoDirectRefsQuery();
  const [fetchThreeToFiveRefsUsers, threeToFiveResult] = useLazyGetUsersWithThreeToFiveDirectRefsQuery();
  const [fetchSixToNineRefsUsers, sixToNineResult] = useLazyGetUsersWithSixToNineDirectRefsQuery();
  const [fetchTenToTwentyFiveUsers, tenToTwentyFiveResult] = useLazyGetUsersWithTenToTwentyFiveDirectRefsQuery();
  const [fetchTwentySixToHundredUsers, twentySixToHundredResult] = useLazyGetUsersWithTwentySixToHundredDirectRefsQuery();

  const [sendBulkNotification] = useSendBulkNotificationMutation();

  // Map each category id → { result, trigger }
  const CATEGORY_MAP = {
    "all-users": { result: allUsersResult, trigger: fetchAllUsers },
    "inactive": { result: inactiveResult, trigger: fetchInactiveUsers },
    "zero-refs": { result: zeroRefsResult, trigger: fetchZeroRefsUsers },
    "1-2-refs": { result: oneToTwoResult, trigger: fetchOneToTwoRefsUsers },
    "3-5-refs": { result: threeToFiveResult, trigger: fetchThreeToFiveRefsUsers },
    "6-9-refs": { result: sixToNineResult, trigger: fetchSixToNineRefsUsers },
    "10-25-refs": { result: tenToTwentyFiveResult, trigger: fetchTenToTwentyFiveUsers },
    "26-100-refs": { result: twentySixToHundredResult, trigger: fetchTwentySixToHundredUsers },
  };

  const getCurrentCategoryData = () => {
    const entry = CATEGORY_MAP[selectedCategory];
    if (!entry) return { users: [], isLoading: false, refetch: () => { }, error: null };

    const { result, trigger } = entry;
    return {
      users: result.data?.data || [],
      isLoading: result.isLoading || result.isFetching,
      refetch: trigger,   // calling the lazy trigger again re-fetches
      error: result.error,
    };
  };

  const { users: currentUsers, isLoading: currentLoading, refetch: currentRefetch, error: currentError } =
  ...
  const handleCategorySelect = (categoryId) => {
    ...
    // Fire the lazy fetch for the chosen category
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {USER_CATEGORIES.map((category) => (
              <CategoryCard
                ...
                category={category}
                isSelected={selectedCategory === category.id}
                isLoading={selectedCategory === category.id && currentLoading}
                userCount={selectedCategory === category.id ? userCount : null}
                onClick={() => handleCategorySelect(category.id)}
              />
            ))}
...
const CategoryCard = ({ category, isSelected, isLoading, userCount, onClick }) => {
  const Icon = category.icon;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center text-center">
        <div
          className={`w-14 h-14 rounded-full flex items-center justify-center mb-3 transition-colors ${isSelected ? "bg-[#b9fd5c]" : "bg-[#111214] group-hover:bg-[#b9fd5c]/10"
            }`}
        >
          {isLoading ? (
        ...
        {isSelected && userCount !== null && !isLoading && (
          <span className="mt-3 bg-[#b9fd5c] text-black text-[11px] font-semibold px-3 py-1 rounded-full flex items-center gap-1">
            <CheckCircle size={12} />
            Selected • {userCount} users
          </span>
        ...
        {isSelected && isLoading && (
          <span className="mt-3 text-[#b9fd5c] text-[11px]">Loading...</span>
        )}
