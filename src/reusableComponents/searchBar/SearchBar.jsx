...
const SearchBar = ({
  ...
  const handleChange = (e) => {
    ...
    // ✅ Hide typing indicator after debounce delay
  ...
  // ✅ Show loader when typing (debounce pending) or when API is loading
  const showLoader = isTyping || isLoading;
  ...
  return (
    <div className={`relative w-full sm:w-52 ${className}`}>
      ...
      {showLoader ? (
        <Loader
          size={15}
          className="absolute left-3 top-1/2 -translate-y-1/2 text-[#b9fd5c] animate-spin pointer-events-none"
        />
      ) : (
        <Search
          size={15}
      ...
      {/* ✅ Right side — Clear button or nothing */}
      {value && (
        <button
          type="button"
          onClick={handleClear}
